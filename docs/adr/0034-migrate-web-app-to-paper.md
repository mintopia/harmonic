...
- Vite plus Tailwind v4. A single `web/src/index.css` (about 446 lines) defines
...
- Home is `components/Deck.tsx` (about 705 lines) plus `deck-model.ts`/`board-model.ts`,
...
- `components/TicketPage.tsx` (about 981 lines) is still a tabbed shape; run-rail
...
after every slice. Delivered as GitHub epic plus eight `ready-for-agent` children:

1. Paper token foundation plus contrast gate: revalue every `--hm-*` (three times) to
   Paper; add `await`, `on-await`, `on-done`, `sunken`, `edge-strong`; a `.bold`
   density layer plus tint washes; `@theme` radii 8/10/13. Update `contrast.test.ts`
...
2. Two-voice plus merged vocabulary: indigo `await` becomes the
...
3. Shell: `App.tsx` landmarks (`nav`/`main`/`header`/`aside`); Soft↔Bold
   toggle (persisted) plus theme cycle; rail labelled "Board"; indigo needs-you badge.
4. Board home as card strips: replace the panelled row-lists with Needs-you /
   Active horizontal card strips plus first-class Standalone cards; a Card with the
   colored left accent bar, HITL/epic badges, branch ref, ctx%, and the
   "→ N more" overflow affordance.
5. Epic frontier-DAG: a new `epic-frontier-model.ts` (Frontier plus Depth
   ...
   (one green).
6. Ticket run rail plus Run-OR-Changes: rework `TicketPage` from tabs to the
...
7. Cross-cutting a11y plus motion pass: `aria-live` regions (polite state plus
...
8. Retire Deck plus refresh design artifacts: delete Deck-only dead code, drop
...
- The app is visually Paper from c1 onward even though the surfaces reshape over
  c4 to c6, an intentional "coloured but not yet re-shaped" interim, not a bug.
