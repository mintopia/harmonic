...
- Two missing human-in-the-loop escape hatches for the escalate/block verification outcomes now exist, avoiding about $6 builder re-runs that also discard the candidate.
...
- Landing an adopted run required a new terminal disposition `operator-accept`, ranked `operator-cancel > operator-accept > escalate`, so an operator's explicit Accept overrides the automatic `escalate` still on the reused run's fact log (reliability-design section 0.3 updated). ADR-0002's journaled-landing mechanism is unchanged; only the disposition precedence table gained a rank.
