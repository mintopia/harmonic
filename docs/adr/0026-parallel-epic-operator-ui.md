...
whole-Epic land) but shipped with only a `force-land` endpoint. There was no read
...
- Representation is board-hosted, not a new rail. Epic-child Tasks group into
...
- Force-land is an armed operator action. An `ArmedButton` (arm then confirm)
...
- Data source is a new server-side read endpoint. `GET …/epics` and
  `GET …/epics/:ref` compose the pure derivations (`deriveEpics` and
...
- Read endpoint plus dedicated `epic_changed`/train-transition firehose events.
...
- The Epic peek is a net-new component; there is no member-detail surface to
  build, since rows reuse `TaskDetail`.
