...
sending a steering message: `POST /api/tasks/:id/steer` with `{ text }`. When the
...
- `POST /tasks/:id/steer` is **operator-only** — excluded from `scopedKeyAllowed`
  ...
  itself.
- The former "inherent, tiny race" — a steer arriving after the loop broke to
  settle was silently queued and dropped — is now closed. `ActiveRun.steerable`
...
the *existing* prompt await in `Runner.drive` — no change to the drive loop's
