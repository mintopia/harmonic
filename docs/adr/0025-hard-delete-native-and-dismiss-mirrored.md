...
- `TaskService.delete(id)` cascades in one transaction, children first (runtime
  ...
  `api_keys` rows), then `runs`, then the Runs' orphaned `sessions`, then `task_channels`,
  then `task_dependencies` (both directions), then null out any `reattempt_of` pointing at
  this Task (a re-attempt becomes standalone rather than dangling), then the `tasks`
  row, then the tombstone (mirrored only). Former dependents are then re-derived
  (`blocked` to `ready`), matching how every other edge change re-derives.
