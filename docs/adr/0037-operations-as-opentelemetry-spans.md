...
about 18 raw `console.*` sites, no levels, no correlation, no aggregate counts. The
...
- Operation is a span. A discrete atomic action Harmonic's runtime performs is
  ...
  decomposed into Operation spans, since that stays Activity / Usage. (See `CONTEXT.md`,
  Operation.)
...
- Linkage and the live registry. Parent/child linkage is hybrid:
...
- Config via `OTEL_*` env vars plus CLI arguments, resolved at process init in
...
- Providers (Tracer/Logger/Meter) initialise once at process start and
  flush plus shut down gracefully on `SIGTERM` / daemon stop, so in-flight spans
  ...
  `harmonic.run`, `harmonic.epic.land`, `harmonic.merge.member`, and so on); `ERROR`
...
- UI. A new Operations rail, beside Activity (shared live backend, distinct
  ...
  merge/poll leaves a trace. Delivered as a `GET /api/operations` snapshot plus
  `operations` firehose event, consumed snapshot-on-load plus firehose-merge like
...
- A new dependency surface (`@opentelemetry/*`: api, sdk-trace-node, sdk-logs,
  ...
  metrics, and live-view we would otherwise hand-roll.
