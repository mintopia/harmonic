...
shape already expressed in zod). The migration cost — every handler's
ad-hoc `z.object(...).parse(req.body)` moves into declared route
schemas, and response schemas are written for the first time — buys a
spec that cannot disagree with the code plus uniform request validation.
