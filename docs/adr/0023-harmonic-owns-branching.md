...
- Prevent via a git `reference-transaction` hook (rejected for v1). True
  ...
  which is invasive. Revisit only if detection proves insufficient.
...
Decision holds (Harmonic owns branching; enforced by detection). The review
hardened it: validation runs in the `validating` phase before any commit/cleanup;
persist start OID plus dirty fingerprint; afk direct requires a clean context and
...
Landing is journaled and crash-idempotent, in a dedicated admin worktree, with an
...
`docs/reliability-design.md` Unit D and section 0.3.
