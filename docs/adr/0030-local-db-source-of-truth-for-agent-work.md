...
contradictory meaning: `isReady` ("any assignee means not ready") versus `foreignAssignee`
("non-@me means foreign"), and integration-branch liveness is a per-poll set
...
5. Priority is a DB-owned field, ordered: explicit priority (native tickets
   set it; mirrored tickets carry a nullable augmented value), then topological
   (unblocked-first, then unblock-count), then age (created date / number). The pick is
...
6. Status is bidirectional, gated by adapter capability. Harmonic pushes
   ...
   inbound close while a Task still runs is premature, so reopen plus escalate
   ...
   read-only contract; premature-close there is detected as "still-running plus
...
- Explicit divergence from upstream, accepted knowingly. The wayfinder/`to-tickets`
  ...
  `add-assignee @me`). This ADR replaces that with `ready-for-agent` plus local DB, and
...
- Requires a migration (fact columns plus priority plus skip-reason) and a scheduler
