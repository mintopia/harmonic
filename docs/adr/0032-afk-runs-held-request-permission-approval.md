...
- **Conversations** (ADR-0007) hold each harness `session/request_permission`
  *open* and prompt the operator, resolving only when they pick an option. An
  opt-in **Permission Rule** (tool kind + Working Directory) auto-approves future
...
ADR-0007 held-request + Permission-Rule approach.
...
1. **afk Runs hold, they do not Escalate on a permission request.** The Runner
   ...
   tool kind + workspace root matches a Permission Rule is auto-approved silently
...
2. **The operator answers out of band, and may remember.** An operator-only
   ...
   it persists a Permission Rule (tool kind + workspace root) via the existing
...
- An afk Run that needs approval pauses and asks instead of dying; once the
  ...
  that repo again — the Rule is shared with Conversations. This is the payoff.
...
- The operator gains a new responsibility surface (pending approvals) and a new
  ...
  or degrades cleanly.
- Rule granularity stays coarse (tool kind + directory), inherited from ADR-0007.
