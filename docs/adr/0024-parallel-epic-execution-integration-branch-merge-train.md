...
does not. The Auto-Runner is already a pool (Machine Ceiling plus per-Workspace cap,
...
- Grouping is a derived Epic, keyed on the tracker parent ref. The
  ...
  tracker: native GitHub sub-issues plus native issue dependencies where present,
...
- A per-Epic integration branch, owned by Harmonic. Harmonic cuts an
  ...
  branch, which requires a new per-Task `baseBranch` (default is today's
...
- Landing is a single-writer merge train per integration branch. One landing
  ...
  bounded corrective turn to rebase and resolve; still conflicting goes to Escalate
...
- Derivation targets the current `/to-spec` convention (native sub-issues plus
  `## Parent` / `## Blocked by` body sections). Older prose-only Maps (body
  task-list plus "Dependency order:" line, no native edges) can't be derived
  cleanly and fall back to today's per-Run behaviour, which is acceptable since
...
- First-class dependency on tracker parent/child fidelity. Missing or messy
  ...
  Authoring correct tickets is the operator's or an agent's job, by design.
