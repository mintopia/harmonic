...
5. **#200 owns the general guarantee**, beyond the driver swap: an event-loop watchdog (shipped as `src/reliability/event-loop-monitor.ts` — `EventLoopMonitor`, named "monitor" not "watchdog" since CONTEXT.md reserves watchdog for the Guardrail; started in `onListen`, off in tests; issue #210), a rule+helper that background loops chunk and yield (shipped as `src/reliability/yield.ts` — `forEachYielding`/`yieldToEventLoop` — with the convention in `AGENTS.md`; issue #211), per-query wall-clock timeouts, and routing heavy aggregates through the async/off-thread path.
