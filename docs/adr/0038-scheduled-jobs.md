...
- A Scheduled Job is a recurring, Harmonic-scheduled background job that fires on
  ...
  (cron-entry to invocation). (See `CONTEXT.md`, Scheduled Job.)
...
- The new drains actually run on a cadence. The Session-retirement drain
  becomes a real interval Job (about 5 min); orphan-worktree reconcile is a new Job
  (about 30 min) that reconciles `git worktree list` against the DB and removes
...
- A narrow, documented divergence from ADR-0031/0037: this schedule state is
  ...
  bookkeeping versus Operation aggregate) is the reason, recorded here so a future
