...
claimed (section 5) a general guarantee that "nothing, slow query or background loop,
...
So ADR-0029 section 5's premise, that making DB access async moved query CPU off the
...
- Treat "async libsql" as not sufficient for the section 5 off-loop guarantee on a
...
- Keep the other section 5 mechanisms unchanged: the single-writer queue, per-query
...
- Amends ADR-0029 section 5: async DB access alone is not off-loop for local
...
None (amends ADR-0029 section 5).
