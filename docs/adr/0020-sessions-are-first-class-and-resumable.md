...
A Session is one ACP conversation with a Harness, 1:1 with the harness's own
`sessionId`. It becomes a first-class, reusable resource. A retry, an automated or
...
never resume), and every retry or reject starts a fresh cold Run, throwing away the
...
- Keep fail-forward, a fresh Run every time (rejected). Safe but wasteful. It
  discards warm context and re-does work a restart interrupted.
- Keep the harness process warm to preserve the live session (rejected). This pins
...
- Assume a fixed 5-minute window because cache TTL is opaque over ACP
  ...
  constant Harmonic sets and therefore knows: about 1 h for Claude,
  5 to 30 min for Codex, about 5 min for Copilot (unverified). It is not one opaque guess.
...
- Forcing the 1h TTL on Claude via `ENABLE_PROMPT_CACHING_1H=1` bills cache
  writes at a higher rate but keeps reads cheap and avoids cold reprocesses. It is
...
- Keepalive: the cache is a sliding window refreshed on each hit, so a cheap
  no-op turn holds a Session warm through a known wait (for example automated
  Verification, ADR-0021). It matters chiefly for the short-window harnesses
  (Codex/Copilot); with Claude's 1h window it is rarely needed. It runs off a
  client-side timer because ACP surfaces no cache signal, and costs one cache-read per
  ping.
- Reject splits by warmth. An automated reject (verify-agent, fires in
  ...
  (reloads full context, costs more) versus fresh (summary).
...
- Cache warmth is a cost signal, not a correctness gate. `session/load` is valid
  warm or cold; warmth only changes cost and latency. Resume eligibility is a
  compatibility matrix; the per-harness window is a cost estimate
  (`estimatedWarmUntil`), never a promise.
- Resume is a new Run plus a new prompt turn on a loaded Session after repo
