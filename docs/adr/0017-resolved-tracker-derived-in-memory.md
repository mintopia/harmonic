...
gates the loop and knows the outcome. Caching it there keeps derived tracker
state in one tier instead of splitting it across a DB column. It sits beside the
poller's own in-memory scan cache (`lastScan`, `urlByRef`) and is surfaced the
same way task `url`/`mapTitle` already are (`server/serialize.ts`).
