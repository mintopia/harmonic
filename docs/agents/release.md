...
4. The `back-merge` job opens a `main` → `develop` PR and auto-merges it, so the
   version bump and CHANGELOG return to the working line with no manual step.
...
- **`main` can't be fast-forwarded.** Each promotion lands a merge commit that
  ...
  never `git push origin develop:main`. The `back-merge` job repairs the
  diverged release commit automatically; a stalled back-merge PR (required
  checks that a `GITHUB_TOKEN` PR never triggers) is the one case to merge by
  hand.
