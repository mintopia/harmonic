  ...
  return (
    <div className="flex gap-4 overflow-x-auto pb-4">
      {columns.map(({ state, terminal, tasks: column }) => {
        ...
        return (
          <section key={state} className="min-w-[180px] flex-1">
          ...
          <div className="flex flex-col">
            {collapsedTerminal.map(({ state, tasks: column }) => (
              <button
                ...
                className="flex items-center justify-between gap-2 rounded-md px-1.5 py-1.5 text-muted transition-colors duration-150 hover:bg-surface hover:text-ink"
                onClick={() => togglePeek(state)}
              >
                {COLUMN_LABELS[state]}
                <span
