    ...
    <aside className="flex shrink-0 flex-col bg-shell max-md:max-w-[50vw]" style={{ width: sidebarWidth }}>
      {panel === 'explorer' ? <>
        ...
        <section aria-labelledby="source-control-title" className="min-h-0 flex-1 overflow-auto">
          ...
          {status.error ? <LoadError message={status.error} onRetry={status.reload} className="m-3" /> : changeCount === 0 && !errors[''] && <p className="px-3 py-3 text-small text-muted">The working tree is clean.</p>}
        </section>
      </>}
    </aside>
