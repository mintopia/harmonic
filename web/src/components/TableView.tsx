...
export function TableView({ onOpen }: { onOpen: (task: Task) => void }) {
  ...
  const [sortBy, setSortBy] = useState<'createdAt' | 'priority' | 'cost'>('createdAt');
  ...
  const sortHeader = (key: 'createdAt' | 'priority' | 'cost', label: string) => (
    <th
        ...
        <tbody>
          {tasks.map((task) => (
            ...
            >
              ...
              <td className="text-xs text-zinc-400">{formatCost(task.cost) ?? '—'}</td>
