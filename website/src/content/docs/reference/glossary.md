...
**Setting Override** — An overridable setting — Task defaults (Harness,
model, Isolation Mode, Priority) and a Workspace's concurrency cap —
resolves as `Workspace value ?? global default`: a Workspace stores
*inherit* until it sets an explicit value, which *reset to default*
clears back to inherit. Global-only settings (Harnesses, prices,
...
dependencies leave dependents blocked and flagged — nothing cascades
automatically.
...
— GitHub, GitLab, or Local Markdown — resolved from its
...
entity — a query-time roll-up over the polled tracker.
...
`drive ≠ hitl`. Mirrored Tasks bypass the review gate entirely —
...
it is unresolved — Auto-Retried, then Escalated.
...
Workspace has it enabled, starts that Workspace's *ready* Tasks —
highest Priority first, FIFO within — up to the Workspace's own
...
**Notification Channel** — A configured destination — Discord webhook,
Slack webhook, Generic webhook, or Email — subscribed to a set of event
types, overridable per Task.
