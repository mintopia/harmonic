...
the API-key management UI and connection info: base URL, the MCP endpoint,
a `curl` example, and download links for the JSON/YAML spec.
...
server mounted at **`POST /mcp`**; Harmonic builds a fresh server instance
per request, so the registered tool list always reflects current config.
...
WebSocket clients can't set headers, so they pass `?token=` instead. Harmonic
shows a created token **once** and stores it only as a hash. There is no way
