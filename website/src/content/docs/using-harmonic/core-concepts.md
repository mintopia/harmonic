...
settings — Harness, model, Working Directory, Isolation Mode — that
...
A **Harness** is the agent CLI Harmonic drives to execute Runs — Claude
Code, Codex, or Copilot — always over the ACP protocol. While a Run is
...
Tasks — highest **Priority** (high / normal / low) first, FIFO within a
...
cascades automatically — if a dependency fails, its dependents simply
stay blocked and flagged until someone intervenes.
