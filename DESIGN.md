<!-- TARGET: this describes the terminal-native redesign direction chosen 2026-07-14.
     ...
     Board-column behavior (dense grid vs collapsible empty columns) is an open
     question to be settled by prototyping variants — see Components > The Board. -->
...
**Key Characteristics:**
- Mono-forward: one typeface, five roles, fixed rem scale
...
**Body Font:** JetBrains Mono (same family throughout)
**Label/Mono Font:** JetBrains Mono — the system is mono-forward by doctrine

**Character:** One engineered monospace carries the entire interface, terminal-native without cosplay: no scanlines, no glow. Hierarchy is built from five fixed rem roles and two weights (400/600), with `tabular-nums` inherent everywhere numbers appear.
...
- **Body** (400, 0.8125rem/1.55): prompts, prose, data cells. Prose blocks cap at 75ch; tables and streams may run full width.
...
**The One Family Rule.** No second typeface, ever. A sans sneaking into a dialog is a regression.
...
### The Board (signature component — variant exploration pending)
Full-width working surface; columns as tonal Surface wells with Label headers and count. Two candidate treatments for the 8-column width problem, to be settled by prototyping both (`$impeccable live` or a throwaway prototype), per the operator's request:
1. **Dense grid** — tighter min column width (~180px), 8px gaps, compact cards, aiming for all columns on one monitor.
2. **Collapsed rails** — empty (or terminal-state) columns collapse to slim vertical bars showing a rotated label + count, expanding on click; occupied columns get the reclaimed width.
Until settled, neither is normative; everything else in this spec applies to both.
...
- **Do** keep every number in `tabular-nums` and every timestamp in the same Body role — live-updating digits must not jitter.
...
- **Don't** use state colors decoratively, introduce a second typeface, or exceed weights 400/600.
