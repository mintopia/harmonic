...
**The Signal Rule.** State colours mean states, and live on the **state/signal layer** — a column lane, a dot, a count pill, a state pill — never smeared across content and never decorative. A colour the operator can't parse as a state is noise. (*API-docs carve-out, retained from the Ledger:* on the developer-facing API reference only, HTTP-method and response-code pills reuse these tints as a redundant second cue to the always-shown verb/code text.) (*Mirrored-card carve-out (2026-08-08, issue #34):* a mirrored Task's `escalated` tag reuses Running amber's tint/ink. An afk→hitl escalation is the operator's cue that autonomous work now needs them — the closest existing meaning to "work in flight, now yours" — so it earns the amber even though the Task lands back in *ready*, not *running*. The one sanctioned non-state use of a state colour. *Extended app-wide (2026-08-17, issue #99):* the `escalated` tag now reuses this amber on **every** surface that shows it — the mirrored Board card and the Activity row — from a single shared `escalatedChip` style (`web/src/ui.ts`), so one concept renders one way and the tag can't fork registers between surfaces. The Activity row previously rendered it in cobalt; that fork is gone.) (*Continuation-cost carve-out (2026-08-19, issue #175):* the reject dialog's continuation-cost chip colours only the *cold* (pricier) full-continuation band in Running amber's tint/ink — harness-attention chrome that says "this continuation will cost you", the same non-state register as the permission-prompt amber band, not a task state. *warm* stays calm-neutral and *unknown* plain, so amber marks the expensive path alone rather than dressing up the cheap one as the promoted choice. *Extended (2026-08-19, issue #177):* the *start-condensed* path now carries a computed band too (`estimateCondensedContinuationCost`), rendered through the same chip — so when the full path is a warm cache hit, the pricier condensed re-prime is the one that wears the amber. The invariant is unchanged: amber lands on whichever of the two re-attempt paths is the pricier one right now, never both.)
...
- **Solid destructive:** Failed-rose fill, `on-fail` label, weight 600 — the loud end of the destructive scale, the filled counterpart of Quiet-destructive. (**Added** 2026-08-17, issue #98.) Reserved for the confirm *inside* a deliberate guard — the type-the-name Workspace-delete modal — where the action is irreversible and the operator has already committed to it; casual destructive links stay Quiet so the loud red never becomes ambient. `--hm-on-fail` (white / near-black) clears the text-contrast floor on the fill in both themes (`tests/contrast.test.ts`); the class is `btnDestructive` in `web/src/ui.ts`.
- **Review gate:** the gate is **two verbs, Accept and Reject** — a reviewed Task merges or fails, and nothing else belongs in the row. Accept is the cobalt primary and sits **last**, so the affirmative holds the terminal position; Reject is the **Ghost** (amended 2026-07-16: this line read "a quiet ghost" while § Task detail read "Reject… (quiet)", naming both roles at once — Ghost wins, because the Quiet role is what a *third* action would take, and there is no third action). Accept is deliberately unguarded (PRODUCT.md: "the review gate is sacred" — the operator's read *is* the review; a worktree conflict returns the Task to *awaiting-review*). Reject opens a dialog because it takes a reason, not because it needs a guard. (Amended 2026-08-19, issue #174 — ratified in grilling #173: Accept stays unguarded when the Verification verdict is *proceed* or absent, but arms into a two-step confirm ("Critic flagged — accept anyway?") when the verdict is *block*/*escalate*. This is not a guard on the review — it is a guard on *overriding a red machine verdict the operator's read may not have seen*, since the critic's verdict otherwise sat unflagged in the Details tab. The footer also inlines a one-line verdict summary so the signal is present where Accept is clicked. `web/src/components/TaskActions.tsx`.)
- **Cancel is not a gate action** (2026-07-16). Once work exists, *cancelled* and *failed* are two names for one terminal fact, and offering both asks the operator to choose between synonyms — while rendering as a second quiet destructive action beside Reject, identical in treatment and different in meaning. Cancel keeps its meaning only where a Task has produced nothing to judge (Draft / Blocked / Ready) or is still producing it (Running). The domain stays permissive: the API can still cancel an awaiting-review Task; the interface simply doesn't offer it. (**Amended** 2026-08-17, issue #98: a *running* Task's cancel is the **armed two-step Cancel button only** — the board's drag-to-Cancelled path excludes Running, so a stray drop can never SIGKILL a working agent with no confirm. Draft / Blocked / Ready still cancel by drag; the button-level meaning above is unchanged. Every completed cancel — button or drag — drops an acknowledgement toast, § Toasts below.)
...
- **Success is acknowledged, quietly** (**added** 2026-08-17, issue #98). The same stack carries a second kind: a completed gate action (Accept / Reject / Cancel) drops a neutral card — Raised fill, ink text, a muted check — naming what happened (`Task #12 cancelled`), so an irreversible click never lands silently. It stays neutral, *not* accept-green: green means a *completed state* in this palette, and an acknowledgement is not a state (the same reason the review gate's Accept spends no state colour). `toastSuccess` in `web/src/toast.tsx`.
