...
- **Mirrored Tasks render their description as Markdown** here (issue #34) — the full tracker-issue body, sanitized, styled under `.markdown` (mono only on code). This is where a ticket's structure reads; the board card stays a clamped title. Native Task prompts stay plain (operator-typed, not Markdown).
