...
describe('PATCH /api/config verification', () => {
  ...
  it('accepts a maxSelfHeals patch and leaves autoAccept untouched', async () => {
    const current = (await server.api('GET', '/api/config')).body;
    expect(current.verify.maxSelfHeals).toBe(1);

    const patched = await server.api('PATCH', '/api/config', { verify: { maxSelfHeals: 3 } });
    expect(patched.status).toBe(200);
    expect(patched.body.verify.maxSelfHeals).toBe(3);
    expect(patched.body.verify.autoAccept).toBe(false);
  });
});
