...
describe('Workspace CRUD (ADR-0008, issue #41)', () => {
  ...
  it('round-trips per-Workspace verifier overrides through PATCH and GET (issue #132)', async () => {
    ...
    expect(created.status).toBe(201);
    // A fresh Workspace inherits both verifiers (null), not write-only holes.
    ...
    const cleared = await server.api('PATCH', `/api/workspaces/${created.body.id}`, { verificationCommand: null });
