...
describe('Workspace CRUD (ADR-0008, issue #41)', () => {
  ...
  it('round-trips per-Workspace staged verifier lists through PATCH and GET', async () => {
    ...
    const set = await server.api('PATCH', `/api/workspaces/${created.body.id}`, {
      ...
      taskPreMergeCritics: [{ prompt: 'review the diff', model: 'claude-opus-5' }],
    });
    expect(set.status).toBe(200);
    expect(set.body.taskPreMergeCommands).toMatchObject([{ command: 'npm', args: ['test'], env: {}, timeoutSeconds: 600 }]);
    expect(set.body.taskPreMergeCritics).toMatchObject([{ prompt: 'review the diff', model: 'claude-opus-5' }]);
  ...
  it('rejects an invalid staged critic override', async () => {
    ...
    const rejected = await server.api('PATCH', `/api/workspaces/${created.body.id}`, { taskPreMergeCritics: [{ prompt: 'review it' }] });
    expect(rejected.status).toBe(400);
    expect(rejected.body.error.message).toContain('model');
    ...
    const accepted = await server.api('PATCH', `/api/workspaces/${created.body.id}`, {
      taskPreMergeCritics: [{ prompt: 'review it', model: 'claude-opus-5' }],
    });
    expect(accepted.status).toBe(200);
    expect(accepted.body.taskPreMergeCritics).toMatchObject([{ prompt: 'review it', model: 'claude-opus-5' }]);
    rmSync(dir, { recursive: true, force: true });
  });
