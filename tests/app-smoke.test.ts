...
function stubFetch(opts: {
  ...
  let update = opts.update ?? { availableVersion: null, armedVersion: null, upgradingVersion: null, dismissedVersion: null, idle: { runningAttempts: 0, mergingOrIntegrating: false, conversationMidTurn: false } };
...
describe('App smoke (issue #452)', () => {
  it('shows the available update banner and dismisses that version', async () => {
    const el = await renderApp({
      ...
      update: { availableVersion: '2.7.0', armedVersion: null, upgradingVersion: null, dismissedVersion: null, idle: { runningAttempts: 0, mergingOrIntegrating: false, conversationMidTurn: false } },
    });
  ...
  it('shows the agent-drain notice and cancel action for an armed update', async () => {
    const el = await renderApp({
      ...
      passwordConfigured: true,
      workspaces: [makeWorkspace()],
      update: { availableVersion: '2.7.0', armedVersion: '2.7.0', upgradingVersion: null, dismissedVersion: null, idle: { runningAttempts: 0, mergingOrIntegrating: false, conversationMidTurn: true } },
    });
  ...
  it('shows the upgrade takeover only while the server reports a swap in progress', async () => {
    const el = await renderApp({
      ...
      passwordConfigured: true,
      workspaces: [makeWorkspace()],
      update: { availableVersion: '2.7.0', armedVersion: '2.7.0', upgradingVersion: '2.7.0', dismissedVersion: null, idle: { runningAttempts: 0, mergingOrIntegrating: false, conversationMidTurn: false } },
    });
  ...
  it('holds the upgrade takeover through a failed status poll and clears it after reconnecting to the new version', async () => {
    const upgrading = { availableVersion: '2.7.0', armedVersion: '2.7.0', upgradingVersion: '2.7.0', dismissedVersion: null, idle: { runningAttempts: 0, mergingOrIntegrating: false, conversationMidTurn: false } };
  ...
  it('arms and cancels an update from the banner', async () => {
    const el = await renderApp({
      ...
      passwordConfigured: true,
      workspaces: [makeWorkspace()],
      update: { availableVersion: '2.7.0', armedVersion: null, upgradingVersion: null, dismissedVersion: null, idle: { runningAttempts: 1, mergingOrIntegrating: false, conversationMidTurn: false } },
    });
