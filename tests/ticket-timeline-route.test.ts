...
describe('GET /api/tasks/:id/timeline (issue #328)', () => {
  ...
  it('folds all persisted ticket events into chronological order', async () => {
    ...
    expect(response.body.events.map((event: { kind: string }) => event.kind)).toEqual([
      'attempt-started', 'run-started', 'verification', 'guardrail', 'escalation', 'verification', 'operator-accept', 'attempt-finished', 'verification', 'run-finished', 'lifecycle',
    ]);
    expect(response.body.events.map((event: { ts: number }) => event.ts)).toEqual([50, 100, 200, 250, 300, 400, 700, 850, 900, 900, expect.any(Number)]);
