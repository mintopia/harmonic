...
describe('evaluateCategory — gating axis', () => {
  it('a high-confidence FAIL blocks', () => {
    const r = evalCat('code_smells', 1.0, 0.9);
    expect(r.zone).toBe('FAIL');
    expect(r.gated).toBe(true);
    expect(r.verdict).toBe('FAIL');
  });

  it('a low-confidence FAIL becomes NEEDS_SIGNOFF, not FAIL', () => {
    const r = evalCat('code_smells', 1.0, 0.4);
  ...
  it('a missing answer scores 0/0 and needs sign-off (FAIL zone, low confidence)', () => {
...
describe('evaluateCategory — ratchet regression', () => {
  ...
  it('flags a drop >= categoryDrop on a gated axis', () => {
...
describe('reason / note builders', () => {
  ...
  function cats(partial: Partial<Record<CategoryId, CategoryResult>>): Record<CategoryId, CategoryResult> {
    const base = Object.fromEntries(
      ALL_CATEGORIES.map((c) => [c, { score: 3.0, confidence: 0.9, zone: 'PASS', gated: true, verdict: 'PASS' } as CategoryResult]),
  ...
  it('blockingReasons lists FAIL, NEEDS_SIGNOFF and ratchet', () => {
    const c = cats({
      code_smells: { score: 1.0, confidence: 0.9, zone: 'FAIL', gated: true, verdict: 'FAIL' },
      duplication: { score: 1.0, confidence: 0.3, zone: 'FAIL', gated: true, verdict: 'NEEDS_SIGNOFF' },
      testability: { score: 2.4, confidence: 0.9, zone: 'WARN', gated: true, verdict: 'WARN', ratchetRegression: { baseline: 3.0, drop: 0.6 } },
    });
  ...
  it('warnNotes lists WARN zones only', () => {
    const c = cats({ code_smells: { score: 2.0, confidence: 0.9, zone: 'WARN', gated: true, verdict: 'WARN' } });
  ...
  it('advisoryNotes flags a low security score with the human-review wording', () => {
    ...
    const c = cats({ security: { score: 1.0, confidence: 0.9, zone: 'FAIL', gated: false, verdict: 'WARN' } });
