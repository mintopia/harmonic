...
import { appendMergeJournalTx, type MergeJournalStore } from './merge-journal.js';
import type { RunSettleCoordinator } from './run-settle.js';
import type { SettleProjection } from './run-coordinator.js';
import { reconcile, type MergeEffect, type ObservedState, type ReconcileAction } from './merge.js';
import type { SpanContext } from '@opentelemetry/api';
import { startOperation } from '../telemetry/operations.js';

/**
 * The journaled, non-interruptible merging operation (issue #115,
 * reliability-design §0.3, Unit D).
 *
 * `RunSettleCoordinator.settle` (run-settle.ts) is *a* precedence decision —
 * it doesn't know or care what side effects a disposition implies. Merging is
 * different: "merge" means actually doing irreversible things (merge a
 ...
 * module makes merging *resumable* instead of atomic: every step is journaled
 * (merge-journal.ts) before/after it happens, and a PONC (Point Of No
 * Cancel — see merge.ts's module doc comment) freezes the Run's disposition
 * the instant the first effect is about to run, so a racing cancel can never
 * again flip the outcome away from a merge already underway.
 *
 * This class owns exactly two operations:
 *   - {@link merge} — drive a fresh merging from its first effect to terminal.
 *   - {@link reconcileMerge} — resume a merging whose process died
 *     mid-flight, by asking the world what actually happened
 *     (`reconcile`, merge.ts) rather than trusting the journal alone.
 *
 ...
 * modelled (`MergeEffect`) but unused until later units add their
...
export class MergeCoordinator {
  constructor(
    ...
    private readonly journal: MergeJournalStore,
  ...
  /**
   * Merge `run`: freeze the PONC, then attempt each effect in order, then
   * settle terminal. `mergeProjection` is the terminal projection this merging
   * intends (matching `RunSettleCoordinator.settle`'s contract) — carried
   * through to the merge `run_fact` and (on success) to the actual settle
   * call. `patch` rides the winning settle write exactly as it does for
   * `RunSettleCoordinator.settle` directly. `mergeFactType` is the disposition
   * kind the merge fact appends as; it defaults to {@link MERGE_FACT_TYPE}
   ...
   *
   *   1. Ensure `run.phase === 'merging'` (recorded + a lifecycle event).
   *   2. Append the merge disposition fact **directly** (NOT through `settle`
   ...
   *      must not see "merged" before the merge (etc.) has actually happened)
   ...
   *      or after this whole unit — it can never slip in between the merge fact
   *      and its PONC row and observe a not-yet-frozen cutoff.
   *   3. With the PONC frozen at the merge fact's seq, that fact is now
   *      guaranteed decisive: any fact any concurrent settle call appends for
   *      this Run — a cancel, a guardrail trip, anything — merges at a strictly
   ...
   *      the log as an audit record, but the merge frozen in at step 2 is what
   *      the operator sees.
   *   4. For each effect, in order: record intent, run `apply()` (bounded by
   *      the operation timeout — see {@link MERGE_OP_TIMEOUT_MS}), record
   ...
   *      merge as decisive — see run-settle.ts), this call idempotently
   *      no-ops (the existing "already not running" guard in `settle`).
   */
  async merge(
    task: TaskRow,
    run: RunRow,
    mergeProjection: SettleProjection,
    effects: readonly MergeEffectExec[],
    patch: Partial<RunRow> = {},
    mergeFactType: RunFactType = MERGE_FACT_TYPE,
    parent = this.opts.parentForRun?.(run.id),
  ): Promise<MergeOutcome> {
    // Parented from the live Run when there is one; an operator Accept on an
    // escalated ticket (Run already settled) merges as its own root operation.
    const operation = startOperation({ type: 'merge', parent, attributes: { 'task.id': task.id, 'run.id': run.id, 'merge.mechanism': 'coordinator' } });
    try {
      const outcome = await operation.run(() => this.mergeUnchecked(task, run, mergeProjection, effects, patch, mergeFactType));
      if (outcome.ok) {
        operation.end();
        await this.opts.onTerminalRun?.(run.id);
      } else {
        operation.fail(outcome.detail ?? 'merging failed');
      }
      return outcome;
    } catch (error) {
  ...
  private async mergeUnchecked(
    task: TaskRow,
    run: RunRow,
    mergeProjection: SettleProjection,
    effects: readonly MergeEffectExec[],
    patch: Partial<RunRow>,
    mergeFactType: RunFactType,
  ): Promise<MergeOutcome> {
    const now = this.opts.now ?? Date.now;

    // Steps 2 + 3 (the PONC freeze) run FIRST, before any other work: append the
    // merge fact and freeze the PONC at its seq — as ONE transaction (ADR-0029).
    // Both `run_facts` and `merge_journal` are now on the async Db, so the two
    ...
    // is strictly ordered relative to it — never interleaved between the merge
    // fact and its PONC row. A settle whose cancel fact merges at a higher seq
    // than this merge fact enqueued its append behind this whole transaction, so
    ...
    await this.db.transaction(async (tx) => {
      const mergeFact = await appendRunFactTx(tx, run.id, mergeFactType, { ...mergeProjection }, now());
      await appendMergeJournalTx(tx, run.id, 'ponc', { payload: { cutoffSeq: mergeFact.seq } }, now());
    });
    ...
    if (run.phase !== 'merging') {
      await this.runStore.update(run.id, { phase: 'merging' });
      await this.runStore.appendEvent(run.id, { type: 'lifecycle', payload: { event: 'phase', phase: 'merging' } });
    }

    const timeoutMs = this.opts.timeoutMs ?? MERGE_OP_TIMEOUT_MS;
    for (const effect of effects) {
      ...
      if (!result.ok) {
        await this.abandon(run, result.detail ?? 'merging failed');
        return { ok: false, detail: result.detail };
      }
    }

    await this.settle.settle(task, run, mergeFactType, mergeProjection, patch);
    return { ok: true };
  }

  /**
   * Record that this merging did not happen: lifts the PONC (`poncCutoff`
   * ignores a frozen cutoff followed by `abandoned`) so the caller's escalate
   * fact can decide the Run instead of the merge fact silently completing a
  ...
  async abandon(run: RunRow, detail: string): Promise<void> {
    ...
    // The Run never merged, so it leaves `merging` for wherever it was — an
    // already-settled (escalated) Run goes back to `terminal`.
    if (run.phase !== 'merging') await this.runStore.update(run.id, { phase: run.phase });
    await this.runStore.appendEvent(run.id, { type: 'lifecycle', payload: { event: 'merge-abandoned', detail } });
  }

  /**
   * Resume a Run's merging journal against the observed world (issue #115):
   * runs {@link reconcile} (merge.ts) over the journal, and for every
   ...
   * entry in `executors` (keyed by `MergeEffect`) if one was supplied —
   ...
   *
   * Deliberately does **not** call `settle` — completing a Run whose merging
   ...
   * Idempotent: reconciling an already-fully-applied merging yields every
  ...
  async reconcileMerge(
    ...
    executors: Partial<Record<MergeEffect, MergeEffectExecutor>> = {},
    ...
    for (const action of actions) {
      ...
      if (action.action === 'adopt') {
        ...
        // merge/PR/close, no false conflict against work that already merged).
...
/**
 * The default disposition type a journaled merging settles under when the
 * caller doesn't name one (issue #191; was the sole, hardcoded kind pre-#191
 * — a successful merging was always `agent-finish/unresolved` with a
 * `completed`-projecting payload, matching `ReviewService.accept`'s pre-#115
 * direct call). Now a parameter (`merge`'s `mergeFactType`) so an operator's
 ...
 */
const MERGE_FACT_TYPE: RunFactType = 'agent-finish/unresolved';

/**
 * The operation timeout for a single merging effect's `apply()` (issue #115).
 * Deliberately independent of any execution-harness timeout — merging runs
 ...
 * (opening a PR, closing a ticket) can override via `MergeCoordinator`'s
 * `timeoutMs` option.
 */
export const MERGE_OP_TIMEOUT_MS = 5 * 60_000;

/** An effect result callback shape shared by {@link MergeEffectExec.apply}
 * and the reconciliation `executors` map. `| undefined` on the optional
 * fields (not just `?:`) so this forwards cleanly into `MergeResult`
 * (merge.ts) under `exactOptionalPropertyTypes` — see that type's doc
 * comment. */
export type MergeEffectOutcome = { ok: boolean; observed?: Record<string, unknown> | undefined; detail?: string | undefined };

/** One merging side effect this operation must apply, in order. `apply` is
...
/** A reconciliation-time re-apply of one effect, keyed by `MergeEffect` in
 * the `executors` map `reconcileMerge` takes — same outcome shape as
 * `MergeEffectExec.apply`, but driven by the journal's recorded identity/
 * expected detail rather than a fresh `MergeEffectExec`. */
export type MergeEffectExecutor = (
  idempotencyKey: string,
  expected: Record<string, unknown>,
) => Promise<MergeEffectOutcome>;

/** What `merge` returns: `ok:true` once every effect applied and the Run
 ...
 * never returns, a hung HTTP call) can't wedge `merge` forever. This does
 ...
 * background. That is precisely the case {@link MergeCoordinator.reconcileMerge}
 * exists to clean up: a later reconcile asks the world (`observed`) and
 * `'adopt's` the effect if it turns out to have merged anyway, rather than
 * re-applying it.
 */
function withTimeout(
  ...
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      if (settled) return;
      settled = true;
      resolve({ ok: false, detail: `merging effect '${effect}' timed out after ${timeoutMs}ms` });
    }, timeoutMs);
    apply().then(
      (result) => {
        if (settled) return;
