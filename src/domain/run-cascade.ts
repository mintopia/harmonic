 ...
 * first — `run_events`, `run_facts`, `landing_journal`, `turn_queue`,
 ...
 * place the run-child set is enumerated, so both `TaskService.delete` (a
 * Task's Runs) and `WorkspaceService.delete` (a whole Workspace's Runs) stay
 * in step and a new FK-to-`runs` table is added in exactly one spot. Sessions
 * (a *parent* of a Run) are deliberately not touched here — each caller
 * decides that, since a Session can be shared across sibling Runs. No-op on
 * an empty `runIds`.
 */
